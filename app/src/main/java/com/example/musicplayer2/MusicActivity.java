...
public class MusicActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private ImageView album,previous, play, pause, next;    // 앨범, 이전, 플레이, 정지, 다음 이미지
    ...
    public void playMusic(MusicDTO musicDTO) {
        try{
            ...
            Uri musicURI = Uri.withAppendedPath( // 음악 파일 Uri 가져오기
                    MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, "" + musicDTO.getId()
            );
            mediaPlayer.reset(); // 음악 재생하기 전에 초기화
            mediaPlayer.setDataSource(this, musicURI); // 음악 데이터 지정
            mediaPlayer.prepare(); // 음악 준비 상태로 돌입
